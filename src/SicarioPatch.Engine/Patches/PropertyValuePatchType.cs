...
namespace SicarioPatch.Engine.Patches
{
    public class PropertyValuePatchType : AssetPatchType<(string ValueType, string Value)>
    {
                ...
                switch (parsedValue.ValueType) {
                    ...
                    case "ByteProperty":
                        if (propertyData.Type == "ByteProperty" && propertyData is BytePropertyData byteProp) {
                            ...
                            catch (HeaderOutOfRangeException) {
        ...
        protected internal override Parser<(string ValueType, string Value)> ValueParser => Parsers.Terms.Identifier().AndSkip(Parsers.Terms.Char(':'))
